...
PeerManager.prototype.addPeer = function(peer, port) {
  if (peer instanceof Peer) {
    ...
    console.log('Node.addPeer(): Invalid value provided for peer', {
      val: peer
    });
...
PeerManager.prototype.checkStatus = function checkStatus() {
  ...
  if (this.peers.length) {
    ...
    console.log(connected + ' of ' + this.peers.length + ' peers connected');
...
PeerManager.prototype.handleVersion = function(e) {
  ...
  console.log("Peer: " + e.peer.version + " " + e.peer.start_height);
  if (!e.conn.inbound) {
    // TODO: Advertise our address (if listening)
    console.log("Advertising our address");
  }
  ...
  if (this.peerDiscovery &&
    ...
    e.conn.sendGetAddr();
    e.conn.getaddr = true;
    console.log("peerdiscovery on");
  }
};

PeerManager.prototype.handleReady = function(e) {
  console.log("connection established");
  console.log('connected to ' + e.conn.peer.host + ':' + e.conn.peer.port);
  this.emit('connect', {
    pm: this,
    conn: e.conn,
    socket: e.socket,
  ...
  if (this.isConnected == false) {
    ...
    console.log("is connected alright");
  }
};
...
PeerManager.prototype.handleAddr = function(e) {
  ...
  e.message.addrs.forEach(function(addr) {
    ...
    console.log("Invalid addr received: " + e.message);
    }
  }.bind(this));
...
PeerManager.prototype.discover = function(options, callback) {
  ...
  var dnsExecutor = seeds.map(function(seed) {
    return function(done) {
      ...
      console.log('resolving dns seed ' + seed);

      dns.resolve(seed, function(err, peers) {
        if (err) {
          console.log('failed to resolve dns seed ' + seed, err);
          self.seeds.failed.push(seed);
          return done(null, []);
        }

        console.log('found ' + peers.length + ' peers from ' + seed);
